...
const LeadDetails = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Fetch lead details
        const leadResponse = await axios.get(`http://localhost:8000/api/lead/${id}/`, {
          headers: {
  ...
  return (
    <div style={{ padding: "20px" }}>
      ...
      {!isEditing ? (
        <div>
          <p><strong>Name:</strong> {`${lead.title} ${lead.first_name} ${lead.last_name}`}</p>
          <p><strong>Email:</strong> {lead.email_address}</p>
          <p><strong>Mobile:</strong> {lead.mobile}</p>
          <p><strong>Office Phone:</strong> {lead.office_phone}</p>
          <p><strong>Job Title:</strong> {lead.job_title}</p>
          <p><strong>Department:</strong> {lead.department}</p>
          <p><strong>Account:</strong> {lead.account_name}</p>
          <p><strong>Status:</strong> {lead.status}</p>
          <p><strong>Status Description:</strong> {lead.status_description}</p>
          <p><strong>Lead Source:</strong> {lead.lead_source}</p>
          <p><strong>Lead Source Description:</strong> {lead.lead_source_description}</p>
          <p><strong>Opportunity Amount:</strong> {lead.opportunity_amount}</p>
          <p><strong>Referred By:</strong> {lead.referred_by}</p>
          <p><strong>Primary Address:</strong> {`${lead.primary_address_street}, ${lead.primary_address_city}, ${lead.primary_address_state}, ${lead.primary_address_postal_code}, ${lead.primary_address_country}`}</p>
          <p><strong>Alternate Address:</strong> {`${lead.alternate_address_street}, ${lead.alternate_address_city}, ${lead.alternate_address_state}, ${lead.alternate_address_postal_code}, ${lead.alternate_address_country}`}</p>
          <p><strong>Description:</strong> {lead.description}</p>
          <p><strong>Assigned To:</strong> {lead.assigned_to_username}</p>
          <button onClick={() => setIsEditing(true)}>Edit Lead</button>
          <button onClick={() => navigate("/leads")} style={{ marginLeft: "10px" }}>
            Back to Leads
          </button>
        </div>
      ) : (
